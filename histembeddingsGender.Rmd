...
# Read the CSV
df <- read.csv("gender_word_counts.csv")

# Group by decade, gender, and word, then sum the counts
aggregated_df <- df %>%
  group_by(decade, gender, word) %>%
  summarise(total_count = sum(count), .groups = "drop")

# Write to CSV without quotes
write.csv(aggregated_df, "aggregated_gender_word_counts.csv", row.names = FALSE, quote = FALSE)
```
...
```{r plot2, warning=FALSE, echo=FALSE, fig.show="hide"}
library(tidyverse)

# Your paired word lists
male_words <- c(
...
word_pairs <- tibble(
  concept = paste0("pair_", seq_along(male_words)),
  male = male_words,
  female = female_words
)

# Load your word count CSV
df <- read_csv("gender_word_counts.csv")

# Filter to only those words in our lists
df_filtered <- df %>%
  filter(word %in% c(male_words, female_words))

# Build pairing table
pair_df <- data.frame(male = male_words, female = female_words)

# Create a lookup for pairing
word_pair_map <- c(setNames(pair_df$male, pair_df$male),
                   setNames(pair_df$male, pair_df$female))

df_paired <- df_filtered %>%
  mutate(pair = word_pair_map[word]) %>%
  mutate(direction = ifelse(gender == "male", "male", "female"),
         count_signed = ifelse(gender == "male", count, -count))

# Aggregate by word and pair (if needed across multiple genres/decades)
plot_df <- df_paired %>%
  group_by(pair, word, direction) %>%
  summarise(count_signed = sum(count_signed), .groups = "drop")

# Plot
ggplot(plot_df, aes(x = count_signed, y = reorder(pair, abs(count_signed)), fill = direction)) +
  geom_bar(stat = "identity", position = "identity") +
  scale_fill_manual(values = c("male" = "#3498db", "female" = "#e74c3c")) +
  scale_x_continuous(labels = abs) +
  labs(
    title = "Mirrored Gendered Word Counts",
    x = "Word Count",
    y = "Word Pair",
    fill = "Gender"
  ) +
